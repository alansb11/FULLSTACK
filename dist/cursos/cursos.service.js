"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CursosService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const curso_entity_1 = require("./entities/curso.entity");
const estudiante_entity_1 = require("../estudiantes/entities/estudiante.entity");
let CursosService = class CursosService {
    cursoRepo;
    estudianteRepo;
    constructor(cursoRepo, estudianteRepo) {
        this.cursoRepo = cursoRepo;
        this.estudianteRepo = estudianteRepo;
    }
    async create(dto) {
        const curso = this.cursoRepo.create({
            nombreCurso: dto.nombreCurso,
            idGrado: dto.idGrado,
            idCarrera: dto.idCarrera,
            idCatedratico: dto.idCatedratico,
            nombreCatedratico: dto.nombreCatedratico,
        });
        if (dto.estudianteIds?.length) {
            curso.estudiantes = await this.estudianteRepo.findBy({
                idEstudiante: (0, typeorm_2.In)(dto.estudianteIds),
            });
        }
        return this.cursoRepo.save(curso);
    }
    findAll() {
        return this.cursoRepo.find({ relations: { estudiantes: true } });
    }
    async findOne(id) {
        const curso = await this.cursoRepo.findOne({
            where: { idCurso: id },
            relations: { estudiantes: true },
        });
        if (!curso) {
            throw new common_1.NotFoundException(`Curso con id ${id} no encontrado`);
        }
        return curso;
    }
    async update(id, dto) {
        const curso = await this.findOne(id);
        Object.assign(curso, {
            nombreCurso: dto.nombreCurso ?? curso.nombreCurso,
            idGrado: dto.idGrado ?? curso.idGrado,
            idCarrera: dto.idCarrera ?? curso.idCarrera,
            idCatedratico: dto.idCatedratico ?? curso.idCatedratico,
            nombreCatedratico: dto.nombreCatedratico ?? curso.nombreCatedratico,
        });
        if (dto.estudianteIds) {
            curso.estudiantes = await this.estudianteRepo.findBy({
                idEstudiante: (0, typeorm_2.In)(dto.estudianteIds),
            });
        }
        return this.cursoRepo.save(curso);
    }
    async remove(id) {
        const curso = await this.findOne(id);
        return this.cursoRepo.remove(curso);
    }
};
exports.CursosService = CursosService;
exports.CursosService = CursosService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(curso_entity_1.Curso)),
    __param(1, (0, typeorm_1.InjectRepository)(estudiante_entity_1.Estudiante)),
    __metadata("design:paramtypes", [typeorm_2.Repository,
        typeorm_2.Repository])
], CursosService);
//# sourceMappingURL=cursos.service.js.map